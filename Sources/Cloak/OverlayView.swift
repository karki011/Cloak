...
struct OverlayView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .cloakFocusInput)) { _ in
            inputFocused = true
        }
    }
        ...
        HStack(spacing: 10) {
            ...
            Menu {
                Button("Start New Session…") {
                    ...
                    NotificationCenter.default.post(name: .cloakOpenContext, object: nil)
                }
                ...
                Divider()
                Button("Settings…") {
                    NotificationCenter.default.post(name: .ghostOpenSettings, object: nil)
                }
                Divider()
                Button("Restart App") { restartApp() }
                Button("Quit GhostOverlay") { NSApp.terminate(nil) }
            } label: {
    ...
    private var modelSwitcher: some View {
        ...
        let isAnthropic = store.provider == .anthropic
        let models = isAnthropic ? SettingsStore.anthropicModels : SettingsStore.openAIModels
        return HStack(spacing: 6) {
            Menu {
                Text("Model").font(.caption).foregroundStyle(.secondary)
                ForEach(models, id: \.self) { m in
                    Button {
                        UserDefaults.standard.set(m, forKey: isAnthropic ? "anthropicModel" : "openAIModel")
                        settingsBump &+= 1
                    } label: {
    ...
    private func exportAll() {
        ...
        panel.nameFieldStringValue = "ghost-export.md"
...
extension Notification.Name {
    static let cloakFocusInput = Notification.Name("cloakFocusInput")
    static let ghostOpenSettings = Notification.Name("ghostOpenSettings")
}

...
enum GhostPermission: CaseIterable, Identifiable {
...
final class PermissionManager: ObservableObject {
    ...
    func isGranted(_ p: GhostPermission) -> Bool {
        switch p {
...
struct PermissionsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Cloak needs a few permissions")
                .font(.headline)
            Text("Grant these once — macOS remembers them for this app.")
                .font(.caption)
                .foregroundStyle(.secondary)

            ForEach(GhostPermission.allCases) { p in
                HStack(spacing: 10) {
...
extension Notification.Name {
    static let ghostOpenPermissions = Notification.Name("ghostOpenPermissions")
}
